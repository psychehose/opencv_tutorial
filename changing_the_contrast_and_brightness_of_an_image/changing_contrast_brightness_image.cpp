...
void basicLinearTransform(const Mat &img, const double alpha_,
                          const int beta_) {
  Mat res;
...
int main(int argc, char **argv) {
  ...
  img_corrected =
      Mat(img_original.rows, img_original.cols * 2, img_original.type());
  img_gamma_corrected =
      Mat(img_original.rows, img_original.cols * 2, img_original.type());
  ...
  createTrackbar("Alpha gain (contrast)", "Brightness and contrast adjustments",
                 &alpha, 500, on_linear_transform_alpha_trackbar);
  createTrackbar("Beta bias (brightness)",
                 "Brightness and contrast adjustments", &beta, 200,
